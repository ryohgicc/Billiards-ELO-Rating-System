...
### Changed
- 积分算法升级为非对称 Elo：胜方加分公式与败方扣分公式独立计算，`winnerDelta + loserDelta` 不再恒为零。同分对局胜方加 30 分、败方扣 30 分；分差 ≥ 400 的强者赢弱者比赛胜方至少加 12 分、败方仅扣 0 至 10 分，本场净拉开分差，让榜首能逐步领先；分差 ≥ 200 的爆冷胜方至少加 50 分、败方至少扣 25 分，分差 ≥ 400 的爆冷胜方至少加 80 分、败方至少扣 40 分；单场 `winnerDelta` 上限 160，单场 `loserDelta` 下限 −160。
