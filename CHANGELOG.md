...
### Changed
- 积分定级期规则调整：只要一场比赛任意一方仍在定级中，就按校准场结算，不触发正式赛的败方动态扣分、连胜延续奖励或终结连胜奖励；双方仍按各自状态使用 `K_cal=150` 或 `K_formal=100`。
...
### Fixed
- 修复超宽屏或浏览器侧栏环境下整站被最大宽度限制成缩略图的问题，页面外壳现在填满可用浏览器宽度。
- 进一步压缩预约页顶部说明区，并让窄宽视口也保持横向紧凑状态条，避免云端数据加载完成后首屏重新变成大面积空白。
