...
### Changed
- 积分算法升级为非对称 Elo：胜方加分公式与败方扣分公式独立计算，`winnerDelta + loserDelta` 不再恒为零。同分对局胜方加 30 分、败方扣 30 分；分差 ≥ 400 的强者赢弱者比赛胜方至少加 15 分、败方仅扣 5 至 10 分，本场净拉开分差，让榜首能逐步领先；分差 ≥ 200 的爆冷胜方至少加 50 分、败方至少扣 25 分，分差 ≥ 400 的爆冷胜方至少加 80 分、败方至少扣 40 分；爆冷败方扣分被约束在「不超过胜方加分的 1.15 倍」之内，避免高分球员一次输球被过度惩罚；单场 `winnerDelta` 上限 160，单场 `loserDelta` 下限 −160。
- 历史比赛由 `replayMatches` 按 `createdAt` 升序自动按新算法回放，部署新代码后下一次 `/api/state` 调用即按新公式重算所有 `matches` 记录的 `winnerDelta`、`loserDelta` 和最终积分，无需手动迁移历史排名或修改 `matches` 表数据。`settings.kFactor` 仍维持 `60`，无新增 D1 迁移。
- `/algorithm` 算法说明页同步重写：胜方与败方公式独立列出，新增同分、强者赢、低分爆冷三个示例表格，并解释非对称设计如何拉开/压缩分差。
