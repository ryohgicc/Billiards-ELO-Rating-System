...
- 积分扣分逻辑改为单人视角半额计算：败方按自己的赛前积分、对手赛前积分和自己的 K 值扣分后再乘 `0.5`，弱输强少扣，强输弱多扣，但整体扣分少于同场加分。
- 积分公式改为标准 Elo 胜方奖励 + 败方半额扣分，同分中段对局为 `+50/-25`；低分爆冷和高分稳定取胜都按期望胜率自然计算，并取消单场 `±160` 封顶。
- `/algorithm` 算法说明页同步重写：新增月赛季、分段 K 因子、标准 Elo 公式和同分/强者赢/低分爆冷示例。
