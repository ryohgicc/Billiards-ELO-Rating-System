...
### Changed
- 积分算法引入分段 K 因子（借鉴 FIDE 国际象棋）：每位球员根据已完成比赛总场数自动选择 K 值——新人段（< 10 场）K=80、中段（10-30 场）K=60、稳定段（≥ 30 场）K=40。胜负双方各自查自己的 K 值，互不干扰。新人变化大、稳定老手变化小，避免老手被偶发结果带飞。
- 重新校准非对称 Elo 参数，让赛后体验更接近「赢了多奖励、输了少惩罚」：同分对局中段 K 胜方 +25、败方 −15（净 +10）；分差 ≥ 400 的强者赢弱者比赛胜方至少 +12、败方仅扣 3 至 10 分；分差 ≥ 200 的爆冷胜方至少加 50 分、分差 ≥ 400 的爆冷胜方至少加 80 分；爆冷败方扣分被约束在「不超过胜方加分」之内，避免高分球员被一次失利过度惩罚。
- 历史比赛由 `replayMatches` 按 `createdAt` 升序自动按新算法回放，每场重新计算双方当时的赛前 K 值，部署新代码后下一次 `/api/state` 调用即按新公式重算所有 `matches` 记录的 `winnerDelta`、`loserDelta` 和最终积分，无需手动迁移历史排名或修改 `matches` 表数据。`settings.kFactor` 仍维持 `60`，无新增 D1 迁移。
- `/algorithm` 算法说明页同步重写：新增分段 K 因子说明表，胜方与败方公式独立列出且包含 K 因子项，新增同分、强者赢、低分爆冷三个示例表格。
